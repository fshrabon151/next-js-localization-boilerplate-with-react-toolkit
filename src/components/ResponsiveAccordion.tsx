  ...
  return (
    <React.Fragment>
      {items.map(({ title, description }, index) => (
        <div
          key={`${title} ${index}`}
          className="border rounded-3xl bg-brandBlue-light
                     duration-300
        ...
        >
          ...
          <button
            ...
            className="w-full text-left flex justify-between items-center md:items-center
                       px-4 py-3 md:px-0 md:py-0
                       focus:outline-none md:cursor-default"
            aria-expanded={openId === index}
            aria-controls={`desc-${index}`}
          >
            <Typography className="font-medium text-[20px]">{title}</Typography>
          ...
          <div
            ...
            `}
          >
            <Typography className="text-[#14282E9E]">{description}</Typography>
          </div>
        </div>
      ))}
    </React.Fragment>
